...
public interface OperacionRepository extends JpaRepository<Operacion,Integer> {
    ...
    @Param("valor") Integer valor, @Param("fecha_hora") String fecha_hora);

    @Modifying
    @Transactional
    @Query(value = "UPDATE operaciones SET id_usuario= :id_usuario, producto= :producto, valor= :valor WHERE id= :id", nativeQuery = true)
    void actualizarOperacion(@Param("id") Integer id, @Param("id_usuario") Integer id_usuario, @Param("producto") String producto, 
